...
export default connect(mapStateToProps)(function CalendarWorker(props) {
    const { user } = props;
    ...
    const saveEvent = React.useCallback(() => {
        ...
        if (isEdit) {
            ...
            // Http.post('Cleaner/UpdateEvent', user,newEvent)
            //     .than(res => {
    ...
    const deleteEvent = React.useCallback((event) => {
        // Http.post('Cleaner/DeleteEvent', user,event)
        //     .than(res => {
    ...
    // useEffect(() => {
    //     Http.get('Cleaner/GetEvents', user)
    //         .than(res => {
